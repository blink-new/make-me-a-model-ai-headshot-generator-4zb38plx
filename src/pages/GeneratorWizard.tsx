  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      for (const file of uploadedFiles) {
        const { publicUrl } = await blink.storage.upload(file, `references/${Date.now()}-${file.name}`)
        imageUrls.push(publicUrl)
      }

      // Generate AI headshots using the uploaded images
      ...
      await blink.db.generations.create({
        userId: user.id,
        style: selectedStyle,
      ...
      toast({
        ...
        description: "There was an error generating your headshots. Please try again.",
        variant: "destructive"
      })
    } finally {
